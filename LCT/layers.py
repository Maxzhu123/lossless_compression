...
from LCT.ops import rms_norm, rms_norm_backward
...
class RMSLinear(torch.autograd.Function):
    ...
    @staticmethod
    @torch.compile()
    def rms_linear_backward(norm_weight, grad, weight, rstd, x, bias_dtype):
    ...
    def backward(ctx, grad):
        ...
        return RMSLinear.rms_linear_backward(norm_weight, grad, weight, rstd, x, ctx.bias_dtype) + (None, None, None, None)
...
class Relu2Linear(torch.autograd.Function):
    ...
    @staticmethod
    @torch.compile()
    def relu2_linear_backward(grad, weight, relu, bias_dtype):
    ...
    def backward(ctx, grad):
        ...
        return Relu2Linear.relu2_linear_backward(grad, weight, relu, ctx.bias_dtype) + (None, None, None, None)
...
def compress_weight(module, buffer=None, distribution=None):
    ...
    if isinstance(module.weight, MyCompressed):
        return
    packed = MyCompressed(module.weight, buffer=buffer, dist=distribution or weight_dist)
...
def named_trainable_tensors(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, (Linear, nn.Embedding)) and isinstance(module.weight, MyCompressed):
