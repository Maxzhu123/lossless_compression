...
def get_luts(table):
    ...
    for pi, p in enumerate(prefixes):
        ...
        # EOF can occupy the first code (notably for constant tensors). Give
        # otherwise unused prefixes a valid symbol, also safe for lookahead.
        curr_val = next(iter(bytes_dict.values()))
        for i in range(256):
            if i in bytes_dict:
...
def encode(data, codec, bytes_per_thread, threads_per_block):
    ...
    return np.frombuffer(bytes(encoded), dtype=np.uint8).copy(), np.packbits(binary_gaps), np.array(output_positions, dtype=np.uint32)
