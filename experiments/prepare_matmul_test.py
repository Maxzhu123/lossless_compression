"""Correctness checks and benchmarks for dense matmul with/without decompression."""
...
from compress.compress import decompress
...
def _benchmark(n: int) -> None:
    """Benchmark one n @ 4n dense matmul with and without decompression."""
    k = 4 * n
    ...
    dense_ms, baseline_ms = _compare((dense, baseline))

    print(
        f"n={n:5d}  dense={dense_ms:7.4f} ms  baseline={baseline_ms:7.4f} ms  "
        f"overhead={(baseline_ms - dense_ms) / dense_ms:6.2%}"
    )
...
def main() -> None:
    """Benchmark dense matmul for several n @ 4n sizes."""
    for n in SIZES:
        _benchmark(n)
