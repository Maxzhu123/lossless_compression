...
# Local copy of Transformers 5.12.0 Qwen3, with explicit LCT component calls.
# This file is maintained locally; do not regenerate it from HF's modular file.
...
class Qwen3RMSNorm(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        if self.lct is not None and self.lct.activations and torch.is_grad_enabled():
            return RMSNormFunction.apply(hidden_states, self.weight, self.variance_epsilon, self.lct)
...
class Qwen3MLP(nn.Module):
    ...
    def forward(self, x, input_norm=None):
        settings = self.lct if torch.is_grad_enabled() else None
        if input_norm is not None:
            gate, up = RMSNormGateUpFunction.apply(
                x, input_norm.weight, input_norm.variance_epsilon, settings,
                self.gate_proj.weight, self.up_proj.weight)
        else:
            gate, up = self.gate_proj(x), self.up_proj(x)
        if torch.is_grad_enabled():
            return SwiGLULinearFunction.apply(gate, up, self.down_proj.weight, settings)
...
class Qwen3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if input_norm is not None:
            q, k, v = RMSNormQKVFunction.apply(
                hidden_states, input_norm.weight, input_norm.variance_epsilon,
                self.lct if torch.is_grad_enabled() else None,
                self.q_proj.weight, self.q_proj.bias, self.k_proj.weight, self.k_proj.bias,
...
class Qwen3DecoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        # Fully Connected
        residual = hidden_states
        hidden_states = self.mlp(hidden_states, input_norm=self.post_attention_layernorm)
        hidden_states = residual + hidden_states
        return hidden_states
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        if self.lct is not None and self.lct.activations and torch.is_grad_enabled():
            if attention_mask is not None or past_key_values is not None or use_cache or self.has_sliding_layers:
                raise ValueError("LCT attention requires unpadded full causal training with use_cache=False")
