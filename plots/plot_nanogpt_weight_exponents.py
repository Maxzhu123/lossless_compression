"""Plot early nanoGPT weight exponent evolution using the momentum figure format."""
from pathlib import Path

import numpy as np

from plot_momentum_distribution import load, build, OUTPUT_DIR
...
if __name__ == "__main__":
    ...
    render({"nanogpt_weight_evolution.pdf": (load(path), load(CHECKPOINT_CSV))},
           build_weights, output_dir=OUTPUT_DIR, wide=True, show=False)
    print(f"Saved {OUTPUT_DIR / 'nanogpt_weight_evolution.pdf'}")
